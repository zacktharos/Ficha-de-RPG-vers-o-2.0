...
export const useCharacterSheet = () => {
    ...
    const saveFichasToStorage = useCallback((fichasToSave: Record<string, Ficha>) => {
     ...
     const updateFicha = useCallback((id: string, updatedFichaData: Partial<Ficha>) => {
        setFichas(prevFichas => {
            ...
            // Recalculate level and points based on experience
            ...
            // Recalculate total weight from inventory
    ...
    const updateGmAdjustment = useCallback((attr: keyof Ficha, adjustment: number) => {
        ...
        if (current) {
            ...
            if (adjustment === 0) {
                 delete newAdjustments[attr]; // Clean up if adjustment is zero
            }
    ...
    return {
        ...
        recomecarFicha,
        resetAesthetics,
        calcularAtributos: (ficha: Ficha) => updateFicha(ficha.id, {}), // Recalculates
        diceHistory,
        rollDice,
        clearDiceHistory,
        selectedAttribute,
