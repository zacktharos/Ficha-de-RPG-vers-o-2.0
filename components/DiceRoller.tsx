...
const DiceRoller: React.FC<DiceRollerProps> = ({ onRoll, selectedAttribute, setSelectedAttribute, ficha }) => {
    ...
    return (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 bg-stone-900/90 backdrop-blur-md p-3 rounded-lg shadow-2xl border border-stone-700 w-64 flex flex-col gap-3" style={{ backgroundColor: ficha.darkMode ? 'rgba(20,20,20,0.9)' : ''}}>
            ...
            <div className="max-h-40 overflow-y-auto space-y-2 pr-2 border-y border-stone-700 py-2 rounded-md" style={{ backgroundColor: 'white', color: 'black' }}>
                {Object.entries(diceRollerAttributeGroups).map(([groupName, attrs]) => (
                    <div key={groupName} className="px-1">
                        <h4 className="text-xs font-bold mb-1" style={{ color: ficha.accentColor || '#f59e0b' }}>{groupName}</h4>
                        {attrs.map(attr => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={`flex justify-between items-center text-sm p-1 rounded cursor-pointer ${isSelected ? 'bg-amber-200' : 'hover:bg-gray-200'}`}
                                >
